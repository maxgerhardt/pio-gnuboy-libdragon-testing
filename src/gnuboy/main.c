...
int gnuboy_main(int argc, char *argv[])
{
	int i, ri = 0, sv = 0;
	...
	for (i = 1; i < argc; i++)
	{
		...
		{
			if (i + 2 >= argc) gnuboy_die("missing arguments to bind\n");
		...
		{
			if (i + 1 >= argc) gnuboy_die("missing argument to source\n");
